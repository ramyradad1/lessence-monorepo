...
-- Allows distinguishing between 'original' and 'simulation' perfumes

ALTER TABLE public.products
ADD COLUMN IF NOT EXISTS product_type TEXT DEFAULT 'original'
CHECK (product_type IN ('original', 'simulation'));

-- Set all existing products to 'original'
UPDATE public.products SET product_type = 'original' WHERE product_type IS NULL;

-- Index for filtering
CREATE INDEX IF NOT EXISTS idx_products_product_type ON public.products(product_type);

-- Update search_products RPC to support product_types filter
...
BEGIN
    ...
    WITH matching_products AS (
        ...
        -- Product type (original/simulation)
        AND (
            product_types IS NULL OR array_length(product_types, 1) IS NULL OR
            p.product_type = ANY(product_types)
        )
        -- Rating
        AND (
    ...
    filtered_variants AS (
        ...
        JOIN matching_products mp ON mp.id = pv.product_id
    ...
    legacy_inventory AS (
        ...
        FROM inventory i
        JOIN matching_products mp ON mp.id = i.product_id
        GROUP BY i.product_id
    )
    SELECT p.*
    FROM matching_products p
