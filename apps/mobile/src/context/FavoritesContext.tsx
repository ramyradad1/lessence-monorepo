...
import { useAuth, useFavorites as useSharedFavorites, FavoritesStorage } from '@lessence/supabase';
...
const mobileFavoritesStorage: FavoritesStorage = {
  ...
  async setGuestFavorites(items: string[]): Promise<void> {
    try { await AsyncStorage.setItem(FAVORITES_KEY, JSON.stringify(items)); } catch { /* noop */ }
  },
  async clearGuestFavorites(): Promise<void> {
    try { await AsyncStorage.removeItem(FAVORITES_KEY); } catch { /* noop */ }
  },
};
