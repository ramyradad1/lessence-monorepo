    ...
    <SafeAreaView className="flex-1 bg-background-dark">
      ...
      >
        <View className="flex-row items-center justify-between px-6 py-4 border-b border-white/5">
          ...
          <Text className="text-xl font-display text-white">Manage Order</Text>
          <View className="w-8" />
        </View>
        ...
        <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false}>
          ...
          <View className="mb-8">
            ...
            <Text className="text-white font-bold text-lg mb-4">{order.customer_name || 'Customer'}</Text>
            
            {/* Status Picker (Simulated as row of chips) */}
            <Text className="text-white/40 text-[10px] uppercase font-bold tracking-widest mb-3">Change Status</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row">
              {ALL_STATUSES.map(status => (
                <TouchableOpacity
                  ...
                  className={`px-4 py-2 rounded-full mr-2 border ${order.status === status ? 'bg-primary border-primary' : 'bg-surface-dark border-white/10'}`}
                >
          ...
          <View className="bg-surface-dark border border-white/5 rounded-3xl p-6 mb-8">
            <Text className="text-white font-bold uppercase tracking-widest text-[10px] mb-4">Live Timeline</Text>
            <OrderTimeline currentStatus={order.status} history={order.status_history} />
          </View>

          {/* Admin Notes */}
          <View className="mb-8">
            <Text className="text-white font-bold uppercase tracking-widest text-[10px] mb-4">Internal Notes</Text>
            ...
            <View className="bg-surface-dark border border-white/5 rounded-3xl p-5 mb-4">
              <TextInput
                ...
                multiline
                numberOfLines={3}
                className="text-white text-sm mb-4 min-h-[80px]"
              ...
              >
                {isAddingNote ? <ActivityIndicator size="small" color="black" /> : <Text className="text-black font-bold uppercase tracking-widest text-[10px]">Add Note</Text>}
              </TouchableOpacity>
            </View>
              ...
              {order.admin_notes?.map((note: OrderAdminNote) => (
                <View key={note.id} className="bg-white/5 border border-white/5 rounded-2xl p-4">
                  <View className="flex-row justify-between items-center mb-2">
                    <Text className="text-[#f4c025] font-bold text-[10px] uppercase tracking-widest">{note.admin_name || 'Admin'}</Text>
                    <Text className="text-white/20 text-[8px]">{new Date(note.created_at).toLocaleString()}</Text>
                  </View>
                  <Text className="text-white/80 text-xs">{note.note}</Text>
                </View>
              ))}
            </View>
          </View>
