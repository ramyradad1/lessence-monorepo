...
export default function AdminOrderDetailScreen({ route, navigation }: any) {
  ...
  const { fetchOrderDetail, updateOrderStatus, addOrderNote } =
  ...
  return (
    <SafeAreaView className="flex-1 bg-background-dark">
      ...
      >
        <View
          className={`flex-row items-center justify-between px-6 py-4 border-b border-white/5 ${isRTL ? "flex-row-reverse" : ""}`}
        >
        ...
        >
          ...
          <View className="mb-8">
            <Text
              className={`text-white/40 text-[10px] uppercase font-bold tracking-widest mb-1 ${isRTL ? "text-right" : "text-left"}`}
            >
              {t("admin:order")} #{order.id.slice(0, 12)}
            </Text>
            <Text
              className={`text-white font-bold text-lg mb-4 ${isRTL ? "text-right" : "text-left"}`}
            >
              {order.customer_name || t("admin:anonymous_user")}
            </Text>

            {/* Status Picker (Simulated as row of chips) */}
            <Text
              className={`text-white/40 text-[10px] uppercase font-bold tracking-widest mb-3 ${isRTL ? "text-right" : "text-left"}`}
            >
            ...
            >
              {ALL_STATUSES.map((status) => (
                ...
                >
                  <Text
                    className={`text-[10px] font-bold uppercase tracking-widest ${order.status === status ? "text-black" : "text-white/60"}`}
                  >
                    {t(`admin:${status}`)}
                  </Text>
                </TouchableOpacity>
            ...
            <View className="space-y-4">
              {order.admin_notes?.map((note: OrderAdminNote) => (
                ...
                >
                  ...
                  >
                    <Text className="text-[#f4c025] font-bold text-[10px] uppercase tracking-widest">
                      {note.admin_name || t("admin:admin")}
                    </Text>
                    <Text className="text-white/20 text-[8px]">
                      {new Date(note.created_at).toLocaleString(locale)}
                    </Text>
                  </View>
                  <Text
                    className={`text-white/80 text-xs ${isRTL ? "text-right" : "text-left"}`}
                  >
                    {note.note}
                  </Text>
                </View>
