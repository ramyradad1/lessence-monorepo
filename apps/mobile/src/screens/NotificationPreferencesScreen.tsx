...
export default function NotificationPreferencesScreen() {
  ...
  const { preferences, loading, updatePreferences } =
    useNotificationPreferences(supabase, user?.id);
  ...
  return (
    <SafeAreaView className="flex-1 bg-background-dark">
      ...
      <ScrollView className="flex-1 px-4 py-6">
        <Text
          className={`text-white/40 text-[10px] uppercase tracking-widest font-bold mb-4 ml-2 ${isRTL ? "text-right mr-2" : ""}`}
        >
...
function PreferenceItem({
  ...
  return (
    <View
      className={`flex-row items-center p-5 ${disabled ? "opacity-40" : ""} ${isRTL ? "flex-row-reverse" : ""}`}
    >
      <View
        className={`w-10 h-10 rounded-full bg-white/5 items-center justify-center ${isRTL ? "ml-4" : "mr-4"}`}
      >
        <MaterialIcons
          name={icon as any}
          size={20}
          color={value ? "#f4c025" : "rgba(255,255,255,0.4)"}
        />
      </View>
      <View className="flex-1">
        <Text
          className={`text-white font-display text-base mb-0.5 ${isRTL ? "text-right" : "text-left"}`}
        >
          {title}
        </Text>
        <Text
          className={`text-white/40 text-[10px] leading-relaxed ${isRTL ? "text-right" : "text-left"}`}
        >
          {description}
        </Text>
      </View>
