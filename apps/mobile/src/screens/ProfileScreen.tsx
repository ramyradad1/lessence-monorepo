...
export default function ProfileScreen({
  ...
  return (
    <SafeAreaView className="flex-1 bg-background-dark">
      ...
      >
        <Text
          className={`font-display text-3xl text-white mb-8 ${isRTL ? "text-right" : "text-left"}`}
        >
        ...
        <View className="bg-surface-dark border border-white/5 rounded-2xl p-6 items-center mb-6">
          ...
          <Text className="text-lg font-display text-white mb-1">
            {profile.full_name || t("profile:valued_client")}
          </Text>
          <Text className="text-white/40 text-xs tracking-widest mb-6">
            {profile.email}
          </Text>
          ...
          >
            ...
            />
            <Text className="text-white/60 text-[10px] uppercase tracking-widest font-bold">
              {t("profile:log_out")}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        {[
          ...
          <View className="bg-primary/5 border border-primary/20 rounded-2xl p-6 mb-6">
            <View
              className={`flex-row items-center mb-2 ${isRTL ? "flex-row-reverse" : ""}`}
            >
              <MaterialIcons
              ...
              />
              <Text
                className={`text-white font-bold text-xs uppercase tracking-widest ${isRTL ? "mr-2" : "ml-2"}`}
              >
                {t("profile:administrator")}
              </Text>
            </View>
            <Text
              className={`text-white/40 text-xs mb-4 ${isRTL ? "text-right" : "text-left"}`}
            >
        ...
        <View className="bg-surface-dark border border-white/5 rounded-2xl p-6 mb-6">
          ...
          >
            ...
            />
            <Text
              className={`text-white font-display text-lg ${isRTL ? "mr-3" : "ml-3"}`}
            >
              {t("profile:language")}
            </Text>
          </View>
          <View className={`flex-row gap-2 ${isRTL ? "flex-row-reverse" : ""}`}>
            <TouchableOpacity
              onPress={async () => {
                const isCurrentlyAr = i18n.language === "ar";
                await i18n.changeLanguage("en");
                if (isCurrentlyAr) {
                  Alert.alert(
                    t("common:restart_required", "Restart Required"),
                    t(
                      "common:restart_for_rtl",
            ...
            >
              <Text
                className={`font-bold ${i18n.language === "en" ? "text-black" : "text-white"}`}
              >
                English
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={async () => {
                const isCurrentlyEn = i18n.language === "en";
                await i18n.changeLanguage("ar");
                if (isCurrentlyEn) {
                  Alert.alert(
                    t("common:restart_required", "إعادة التشغيل مطلوبة"),
                    t(
                      "common:restart_for_rtl",
            ...
            >
              <Text
                className={`font-bold ${i18n.language === "ar" ? "text-black" : "text-white"}`}
              >
                العربية
              </Text>
            </TouchableOpacity>
        ...
        <View className="space-y-4 mb-10">
          ...
          >
            <View
              className={`flex-row items-center justify-between mb-2 ${isRTL ? "flex-row-reverse" : ""}`}
            >
              <View
                className={`flex-row items-center ${isRTL ? "flex-row-reverse" : ""}`}
              >
          ...
          <View className="bg-surface-dark border border-white/5 rounded-2xl p-6">
            <TouchableOpacity
              ...
              className={`flex-row items-center justify-between mb-4 border-b border-white/5 pb-4 ${isRTL ? "flex-row-reverse" : ""}`}
            >
              <View
                className={`flex-row items-center ${isRTL ? "flex-row-reverse" : ""}`}
              >
