  ...
  if (error || !bundle) {
    return (
      <View className="flex-1 bg-background-dark items-center justify-center p-6">
        ...
        <Text
          className={`text-white text-lg font-bold mt-4 ${isRTL ? "text-right" : "text-left"}`}
        >
          {t("bundle_not_found")}
        </Text>
        <TouchableOpacity
          onPress={() => navigation.goBack()}
          className="mt-6 bg-white/5 px-6 py-3 rounded-xl"
        >
    ...
    <SafeAreaView className="flex-1 bg-background-dark">
      <View className="flex-1 w-full mx-auto relative">
        ...
        >
          ...
          <TouchableOpacity
            className="flex h-10 w-10 items-center justify-center rounded-full"
            onPress={() => toggleFavorite(bundle.id)}
          >
            <MaterialIcons
              name={isFavorite(bundle.id) ? "favorite" : "favorite-border"}
              size={24}
              color={isFavorite(bundle.id) ? "#ef4444" : "white"}
            />
          </TouchableOpacity>
        </View>
        ...
        >
          ...
          <View className="px-4 pb-6">
            <View className="relative w-full aspect-[4/5] rounded-2xl overflow-hidden shadow-sm bg-slate-800">
              ...
              <View
                className={`absolute bottom-4 z-20 ${isRTL ? "right-4" : "left-4"}`}
              >
          ...
          <View className="px-6 flex-col gap-6">
            ...
            <View className="flex-col gap-1">
              ...
              >
                ...
                <View
                  className={`flex-col pt-2 ${isRTL ? "items-start" : "items-end"}`}
                >
            ...
            <View>
              ...
              <View className="flex-col gap-3">
                {bundle.items?.map((item: any, idx: number) => (
                  <View
                    key={item.id || idx}
                    className={`flex-row items-center gap-4 bg-surface-dark border border-white/5 rounded-xl p-3 ${isRTL ? "flex-row-reverse" : ""}`}
                  >
                    <Image
                      source={{ uri: item.product?.image_url }}
                      className="h-16 w-16 rounded-lg bg-slate-800"
                    />
                    <View className="flex-1">
                      <Text
                      ...
                      <Text
                        className={`text-slate-400 text-xs mt-0.5 ${isRTL ? "text-right" : "text-left"}`}
                      >
                        {item.variant
                          ? `${item.variant.size_ml}ml ${item.variant.concentration}`
                          : t("standard_size")}
                      </Text>
                      <View
                        className={`mt-2 bg-primary/10 px-2 py-0.5 rounded border border-primary/20 ${isRTL ? "self-end" : "self-start"}`}
                      >
        ...
        <View className="absolute bottom-0 left-0 right-0 p-4 bg-background-dark/95 border-t border-white/10 z-40">
          ...
          >
            <Text className="font-bold text-lg text-black">
              {isAdding ? t("added_to_bag") : t("add_set_to_bag")}
            </Text>
            <View
            ...
            >
              ...
              <MaterialIcons
                name={isAdding ? "check" : "shopping-bag"}
                size={20}
                color="black"
              />
            </View>
