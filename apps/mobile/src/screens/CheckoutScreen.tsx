...
export default function CheckoutScreen() {
  ...
  const { user } = useAuth();
  const { loyaltyAccount, redeemPoints, isRedeeming } = useLoyalty(
    supabase,
    user?.id,
  );
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (response.error) {
        if (response.error.context && response.error.context.length > 0) {
          ...
          try {
            ...
            throw new Error(
              errorObj.error || t("checkout:error_initiating_checkout"),
            );
          } catch (e) {
        ...
        throw new Error(
          response.error.message || t("checkout:error_initiating_checkout"),
        );
      }
      ...
      Alert.alert(
        t("common:error"),
        err.message || t("checkout:error_unexpected"),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleRedeemPoints = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        Alert.alert(
          t("common:success"),
          t("checkout:redeem_success_msg", {
            points,
            discount: formatCurrency(result.discountAmount, locale),
          }),
        );
      }
    } catch (err: any) {
  ...
  if (cart.length === 0) {
    return (
      <View className="flex-1 items-center justify-center bg-background-dark px-4">
        ...
        />
        <Text className="text-xl font-light text-white mt-6 mb-8 text-center">
  ...
  return (
    <SafeAreaView className="flex-1 bg-background-dark">
      ...
      >
        ...
        <Text
          className={`flex-1 text-xl font-bold text-white ${isRTL ? "mr-2 text-right" : "ml-2"}`}
        >
          {t("checkout:checkout")}
        </Text>
      </View>
      ...
      >
        ...
        <View className="bg-surface-dark p-4 rounded-2xl border border-white/5 mb-6">
          ...
          />
          <View
            className={`flex-row gap-3 mb-3 ${isRTL ? "flex-row-reverse" : ""}`}
          >
            <TextInput
        ...
        <View className="bg-surface-dark p-4 rounded-2xl border border-white/5 mb-6">
          ...
          >
            ...
            <View
              className={`w-6 h-6 rounded-md border items-center justify-center ${isGift ? "bg-[#f4c025] border-[#f4c025]" : "bg-transparent border-white/10"}`}
            >
          ...
          {isGift && (
            <View className="mt-4 pt-4 border-t border-white/10">
              ...
              >
                <View
                  className={`w-5 h-5 rounded border items-center justify-center ${isRTL ? "ml-3" : "mr-3"} ${giftWrap ? "bg-[#f4c025] border-[#f4c025]" : "bg-transparent border-white/10"}`}
                >
                  {giftWrap && (
              ...
              <View>
                ...
                >
                  ...
                  <Text
                    className={`text-xs ${giftMessage.length > 250 ? "text-red-400" : "text-white/40"}`}
                  >
                ...
                <TextInput
                  ...
                  value={giftMessage}
                  onChangeText={(text) =>
                    setGiftMessage(text.substring(0, 250))
                  }
                  multiline
                  numberOfLines={4}
        ...
        <View className="bg-surface-dark p-4 rounded-2xl border border-white/5 mb-6">
          ...
          <View className="mb-4">
            {cart.map((item, idx) => {
              ...
              return (
                ...
                >
                  <Text
                    className={`text-gray-300 flex-1 ${isRTL ? "text-right pl-4" : "text-left pr-4"}`}
                    numberOfLines={1}
                  >
                    {item.quantity}x {localizedName} ({variantInfo})
                  </Text>
                  <Text className="text-white font-medium pl-2">
                    {formatCurrency(sizePrice * item.quantity, locale)}
                  </Text>
                </View>
              );
            })}
          ...
          <View className="mb-4">
            ...
            {couponError && (
              <Text
                className={`text-red-400 text-xs mt-2 ${isRTL ? "text-right mr-1" : "ml-1"}`}
              >
                {couponError}
              </Text>
            )}
            {discountAmount > 0 && !couponError && (
              <Text
                className={`text-emerald-400 text-xs mt-2 ${isRTL ? "text-right mr-1" : "ml-1"}`}
              >
                {t("checkout:discount_success")}
              </Text>
            )}
            {discountType === "free_shipping" && !couponError && (
              <Text
                className={`text-emerald-400 text-xs mt-2 ${isRTL ? "text-right mr-1" : "ml-1"}`}
              >
                {t("checkout:free_shipping_applied")}
              </Text>
            )}
          </View>
              ...
              >
                ...
                <Text className="text-primary text-xs font-bold">
                  {t("checkout:points_balance", {
                    points: loyaltyAccount.points_balance,
                  })}
                </Text>
              </View>
              <View className={`flex-row ${isRTL ? "flex-row-reverse" : ""}`}>
          ...
          <View className="flex-col gap-2 pt-4 border-t border-white/10 mt-2">
            ...
            {discountAmount > 0 && (
              <View
                className={`flex-row justify-between items-center ${isRTL ? "flex-row-reverse" : ""}`}
              >
                <Text className="text-emerald-400/80 text-xs uppercase tracking-wider">
                  {t("checkout:discount")}
                </Text>
            ...
            {loyaltyDiscount > 0 && (
              <View
                className={`flex-row justify-between items-center ${isRTL ? "flex-row-reverse" : ""}`}
              >
                <Text className="text-primary/80 text-xs uppercase tracking-wider">
                  {t("checkout:loyalty_points")}
                </Text>
                <Text className="text-primary/80">
                  -{formatCurrency(loyaltyDiscount, locale)}
                </Text>
              </View>
            )}
            <View
            ...
            >
              ...
              >
                {discountType === "free_shipping"
                  ? t("checkout:free")
                  : t("checkout:calculated_next_step")}
              </Text>
            </View>
            <View
              className={`flex-row items-center justify-between pt-4 border-t border-white/10 ${isRTL ? "flex-row-reverse" : ""}`}
            >
              <Text className="text-white font-medium">
                {t("checkout:total")}
              </Text>
              <Text className="text-2xl font-bold text-primary">
                {formatCurrency(Math.max(0, orderTotal), locale)}
              </Text>
            </View>
          </View>
        </View>
