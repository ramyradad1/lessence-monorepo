...
export default function HomeScreen() {
  ...
  const { products: supabaseProducts, loading: productsLoading } =
  ...
  const { categories: supabaseCategories, loading: catsLoading } =
    useCategories();
  ...
  const baseProducts =
    supabaseProducts.length > 0 ? supabaseProducts : FALLBACK_PRODUCTS;
  const products =
    searchQuery.length >= 2 || Object.keys(filters).length > 1
      ? supabaseProducts
      : baseProducts;
  const categories =
    supabaseCategories.length > 0 ? supabaseCategories : FALLBACK_CATEGORIES;
  const currentLoading = productsLoading;

  const heroTitle =
    i18n.language === "ar"
      ? banner?.title_ar || banner?.title
      : banner?.title_en || banner?.title || t("common:hero_title_fallback");
  const heroSubtitle =
    i18n.language === "ar"
      ? banner?.subtitle_ar || banner?.subtitle
      : banner?.subtitle_en ||
        banner?.subtitle ||
        t("common:hero_subtitle_fallback");
  const heroDesc =
    i18n.language === "ar"
      ? banner?.description_ar || banner?.description
      : banner?.description_en ||
        banner?.description ||
        t("common:hero_desc_fallback");
  const heroBadge =
    i18n.language === "ar"
      ? banner?.badge_text_ar || banner?.badge_text
      : banner?.badge_text_en ||
        banner?.badge_text ||
        t("common:hero_badge_fallback");
  const heroCta =
    i18n.language === "ar"
      ? banner?.cta_text_ar || banner?.cta_text
      : banner?.cta_text_en ||
        banner?.cta_text ||
        t("common:hero_cta_fallback");
  const heroImage =
    banner?.image_url ||
  ...
  return (
    <SafeAreaView className="flex-1 bg-background-dark">
      ...
      >
        {/* Header */}
        <View
          className={`flex-row items-center justify-between px-4 py-4 bg-background-dark/90 border-b border-white/5 z-20 ${isRTL ? "flex-row-reverse" : ""}`}
        >
          <View
          ...
          >
            ...
            <TouchableOpacity
              className={`p-2 rounded-full ${isRTL ? "mr-1" : "ml-1"}`}
              onPress={() => {
            ...
            >
              ...
              {Object.keys(filters).length > 1 && (
                <View
                  className={`absolute top-1 ${isRTL ? "left-1" : "right-1"} h-2 w-2 rounded-full bg-primary`}
                />
              )}
            </TouchableOpacity>
          </View>
          ...
          >
            ...
            <TouchableOpacity
              className="relative p-2 rounded-full mx-2"
              onPress={() => navigation.navigate("Notifications")}
            >
              <MaterialIcons name="notifications" size={24} color="white" />
              {unreadCount > 0 && (
                <View
                  className={`absolute top-1 ${isRTL ? "left-1" : "right-1"} h-4 w-4 rounded-full bg-primary items-center justify-center border-2 border-background-dark`}
                >
            ...
            >
              ...
              {cartCount > 0 && (
                <View
                  className={`absolute top-1 ${isRTL ? "left-1" : "right-1"} h-5 w-5 rounded-full bg-primary items-center justify-center border-2 border-background-dark`}
                >
        ...
        {isSearchOpen && (
          <View className="px-4 py-3 bg-background-dark border-b border-white/5">
            <View
              className={`flex-row items-center bg-surface-dark border border-white/10 rounded-full px-4 py-2 ${isRTL ? "flex-row-reverse" : ""}`}
            >
              <MaterialIcons
                name="search"
                size={18}
        ...
        >
          ...
          <View
            className={
              isDesktop ? "w-full px-6 pt-6 pb-8" : "w-full px-4 pt-4 pb-6"
            }
          >
            <View
              className={`relative flex-col justify-end overflow-hidden rounded-2xl bg-gray-900 shadow-xl w-full ${isDesktop ? "min-h-[500px]" : "min-h-[420px]"}`}
            >
              <Image
              ...
              <View
                className={`relative z-10 justify-end px-6 pb-10 mt-auto w-full ${isRTL ? "items-end" : "items-center"}`}
              >
                ...
                <Text
                  className={`text-4xl font-bold leading-tight tracking-tight text-white mb-1 ${isRTL ? "text-right" : "text-center"}`}
                >
                  {heroTitle}
                </Text>
                <Text
                  className={`text-4xl font-bold italic text-white/90 mb-4 ${isRTL ? "text-right" : "text-center"}`}
                  style={{ fontFamily: "Newsreader_400Regular_Italic" }}
                >
                  {heroSubtitle}
                </Text>
                <Text
                  className={`max-w-[320px] text-sm font-medium leading-relaxed text-gray-200 mb-6 ${isRTL ? "text-right" : "text-center"}`}
                >
                  {heroDesc}
                </Text>
                <TouchableOpacity
                ...
                >
                  <Text className="text-sm font-bold tracking-wide text-surface-dark">
                    {heroCta}
                  </Text>
                </TouchableOpacity>
              </View>
          ...
          >
            {categories.map((cat) => (
              ...
              >
                <Text
                  className={`text-sm font-semibold ${activeCategory === cat.slug ? "text-white" : "text-gray-400"}`}
                >
                  {i18n.language === "ar"
          ...
          <View className="mt-6 flex-col gap-4">
            ...
            >
              ...
              >
                {searchQuery.length >= 2
                  ? t("common:results_for", { query: searchQuery })
                  : t("common:new_arrivals")}
              </Text>
            ...
            {currentLoading ? (
              ...
              /* DESKTOP: Grid layout */
              <View
                className={`flex-row flex-wrap px-4 gap-4 ${isRTL ? "flex-row-reverse" : ""}`}
              >
                {products.length === 0 ? (
          ...
          <View className="px-4 pb-8 mt-2">
            <View
              className={`relative flex-row w-full items-center justify-between overflow-hidden rounded-xl bg-surface-dark px-6 py-8 ${isRTL ? "flex-row-reverse" : ""}`}
            >
              <View
                className={`relative z-10 flex-col gap-2 flex-1 ${isRTL ? "pl-4" : "pr-4"} ${isRTL ? "items-end" : "items-start"}`}
              >
                ...
                >
                  {t("common:gift_sets_desc")}
                </Text>
                <TouchableOpacity
                  className="mt-2"
                  onPress={() =>
                    navigation.navigate("Shop", { activeCategory: "gift-sets" })
                  }
                >
                  <Text className="text-xs font-bold uppercase tracking-wider text-primary underline">
                    {t("common:shop_gifts")}
                  </Text>
                </TouchableOpacity>
              </View>
              <View className="relative z-10 h-24 w-24">
                <Image
                  source={{
                    uri: "https://lh3.googleusercontent.com/aida-public/AB6AXuBjByiage4rvCg5RzBixD9GZ2zjL2M3mcxYzXubmI8QPTi_XC5Pv9LsCEB453Qlv4qKf__CbDIlUB_0XmgRJswZy0zko4_9VDy_yH2XCGuwmMc0MA8hy0fLovjqHlZ5LR59gkgIgGpXbEJu74jDehx-Kzhs1p8Ksf-4t87u6RvFAAgKhAFvzQ9qtIiiFAS_DUFI5tob_jYNUFWvAmuSvBUeOk9vHrqRk-IkxlHRkWNXsvPu8mvQFor6QVLzKIyNpnKuZSxn2wlsuB64",
                  }}
        ...
        >
          <View
            className={`flex-1 bg-black/60 ${isRTL ? "items-start" : "items-end"}`}
          >
            <View
              className={`h-full bg-background-dark shadow-2xl ${isDesktop ? "w-96" : "w-full"}`}
            >
              <SafeAreaView className="flex-1">
                {/* Modal Header */}
                <View
                  className={`flex-row items-center justify-between p-6 border-b border-white/5 ${isRTL ? "flex-row-reverse" : ""}`}
                >
                  <View
                  ...
                  {lastOrder ? (
                    ...
                    cart.map((item, idx) => (
                      <View
                        ...
                        className={`flex-row gap-4 mb-6 pb-6 border-b border-white/5 ${isRTL ? "flex-row-reverse" : ""}`}
                      >
                        <Image
                          source={{ uri: item.image_url }}
                          className="h-20 w-16 rounded-md bg-surface-dark"
                        />
                        <View className="flex-1">
                          ...
                          <Text
                            className={`text-gray-500 text-xs mt-1 ${isRTL ? "text-right" : "text-left"}`}
                          >
                            {i18n.language === "ar"
        ...
        >
          <View className="flex-1 bg-black/60 justify-end">
            <View
              className="bg-background-dark border-t border-white/10 rounded-t-3xl h-[85%]"
              style={
                isDesktop
                  ? {
                      width: 400,
            ...
            >
              <View
                className={`flex-row items-center justify-between p-6 pb-4 border-b border-white/5 ${isRTL ? "flex-row-reverse" : ""}`}
              >
                <Text
                  className={`text-xl font-bold text-white tracking-widest uppercase ${isRTL ? "text-right" : "text-left"}`}
                >
                  {t("shop:filters")}
                </Text>
                <TouchableOpacity
              ...
              <ScrollView className="flex-1 px-6 py-4">
                {/* Sort Options */}
                <Text
                  className={`text-xs font-bold tracking-widest uppercase text-white/50 mb-3 ${isRTL ? "text-right" : "text-left"}`}
                >
                  {t("shop:sort_by")}
                </Text>
                <View
                  className={`flex-row flex-wrap gap-2 mb-8 ${isRTL ? "flex-row-reverse" : ""}`}
                >
                  {[
                    ...
                    >
                      <Text
                        className={`text-xs font-bold ${draftFilters.sortBy === sort.value ? "text-black" : "text-white/70"}`}
                      >
                        {sort.label}
                      </Text>
                    </TouchableOpacity>
                  ...
                  {[
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        setDraftFilters((prev) => ({
                          ...prev,
                          genderTargets: prev.genderTargets?.includes(
                            gender.value,
                          )
                            ? []
                            : [gender.value],
                    ...
                    >
                      <Text
                        className={`text-xs font-bold ${draftFilters.genderTargets?.includes(gender.value) ? "text-black" : "text-white/70"}`}
                      >
                        {gender.label}
                      </Text>
                    </TouchableOpacity>
                ...
                <View
                  className={`flex-row items-center gap-4 mb-8 ${isRTL ? "flex-row-reverse" : ""}`}
                >
                  <TextInput
                    ...
                    value={
                      draftFilters.minPrice ? String(draftFilters.minPrice) : ""
                    }
                    onChangeText={(val) =>
                      setDraftFilters((prev) => ({
                        ...prev,
                  ...
                  <TextInput
                    ...
                    keyboardType="numeric"
                    value={
                      draftFilters.maxPrice ? String(draftFilters.maxPrice) : ""
                    }
                    onChangeText={(val) =>
                      setDraftFilters((prev) => ({
                        ...prev,
                ...
                >
                  <Text className="text-sm font-bold tracking-widest uppercase text-white">
              ...
              <View
                className={`p-6 border-t border-white/5 flex-row gap-4 bg-background-dark ${isRTL ? "flex-row-reverse" : ""}`}
              >
                <TouchableOpacity
                  onPress={clearFilters}
                  className="flex-1 items-center justify-center py-4"
                >
                  <Text className="text-white/60 font-bold uppercase tracking-widest text-xs">
                    {t("shop:clear_all")}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={applyFilters}
                  className="flex-1 bg-primary items-center justify-center py-4 rounded-full shadow-lg"
                >
