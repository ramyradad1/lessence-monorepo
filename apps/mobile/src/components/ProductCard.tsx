  ...
  return (
    ...
    >
      ...
      <View className="flex-col gap-1">
        <Text className={`text-base font-semibold text-white ${isRTL ? 'text-right' : 'text-left'}`} numberOfLines={1}>{productName}</Text>
        <Text className={`text-xs text-gray-400 ${isRTL ? 'text-right' : 'text-left'}`}>{productSubtitle}</Text>
        <View className={`mt-1 flex-row items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
          <Text className="text-sm font-bold text-white">{formatCurrency(product.price, locale)}</Text>
          <TouchableOpacity 
