  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-[#181611] flex flex-col items-center justify-center p-4">
        <p className="text-white/40 mb-6 font-display text-xl">Order not found</p>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
            <div>
              ...
              <p className="text-white/40 text-sm italic">Placed on {new Date(order.created_at).toLocaleDateString()}</p>
            </div>

            {canRequestReturn && !returnRequest && (
              <button 
                onClick={() => router.push(`/profile/returns/new/${order.id}`)}
                className="flex items-center justify-center gap-2 bg-white/5 border border-white/10 text-white px-6 py-3 rounded-full font-bold uppercase tracking-widest text-[10px] hover:bg-white hover:text-black transition-all"
              >
                <RotateCcw size={14} />
          ...
          {returnRequest && (
            <div className="glass-effect p-6 rounded-3xl border border-[#f4c025]/20 bg-[#f4c025]/5 mb-10 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-white font-display text-lg">Return Request</h2>
                  <p className="text-white/40 text-xs uppercase tracking-widest font-bold">Status: <span className="text-[#f4c025]">{returnRequest.status}</span></p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-white/20 text-[10px] uppercase tracking-widest">Requested on</p>
                <p className="text-white/60 text-xs font-medium">{new Date(returnRequest.created_at).toLocaleDateString()}</p>
              </div>
            </div>
          )}
          ...
          <div className="glass-effect p-8 rounded-3xl border border-white/5 mb-8">
            <h2 className="text-lg font-display text-white mb-6">Order Status</h2>
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              <div className="glass-effect p-8 rounded-3xl border border-white/5">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-lg font-display text-white">Order Items</h2>
                </div>
                  ...
                  {order.items?.map((item) => (
                    <div key={item.id} className="py-4 flex justify-between items-center gap-4">
                      <div>
                        ...
                        <p className="text-white/40 text-xs">{item.selected_size} × {item.quantity}</p>
                      </div>
                ...
                <div className="mt-6 pt-6 border-t border-white/5 space-y-3">
                  ...
                  <div className="flex justify-between text-lg font-bold">
                    ...
                    <span className="text-[#f4c025]">${order.total_amount.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              {order.is_gift && order.gift_message && (
                <div className="glass-effect p-8 rounded-3xl border border-[#f4c025]/20 bg-[#f4c025]/5">
                  <h3 className="text-xs font-bold text-[#f4c025] uppercase tracking-widest mb-4">Gift Message</h3>
                  <p className="text-white/80 italic font-serif text-lg leading-relaxed">{order.gift_message}</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="glass-effect p-6 rounded-3xl border border-white/5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="text-sm font-bold text-white uppercase tracking-widest">Shipping</h2>
                </div>
                {order.address ? (
                  ...
                  <p className="text-white/40 text-xs">Shipping address not available.</p>
                )}
              </div>
              ...
              <div className="glass-effect p-6 rounded-3xl border border-white/5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="text-sm font-bold text-white uppercase tracking-widest">Payment</h2>
                </div>
                <p className="text-white/60 text-sm">Status: <span className="text-white font-medium uppercase text-[10px] ml-1">{order.status === 'pending' ? 'Pending' : 'Completed'}</span></p>
                <p className="text-white/40 text-[10px] mt-2 uppercase tracking-widest">Card ending in ••••</p>
              </div>
            </div>
          </div>
        </div>
