  ...
  if (!order) {
    return (
      <div className={`min-h-screen bg-[#181611] flex flex-col items-center justify-center p-4 ${rtl ? 'rtl' : 'ltr'}`}>
        <p className="text-fg-muted mb-6 font-sans text-xl">{t('order_not_found')}</p>
      ...
      <div className={`min-h-screen bg-background-dark pt-32 pb-20 px-4 ${rtl ? 'rtl text-right' : 'ltr text-left'}`}>
        <div className="max-w-4xl mx-auto">
          <button 
            ...
            className={`flex items-center gap-2 text-fg-muted hover:text-white transition-colors text-sm uppercase tracking-widest font-bold mb-8 ${rtl ? 'flex-row-reverse' : ''}`}
          >
          ...
          <div className={`flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 ${rtl ? 'md:flex-row-reverse' : ''}`}>
            <div>
              <div className={`flex items-center gap-3 mb-2 ${rtl ? 'flex-row-reverse' : ''}`}>
                <h1 className="font-sans text-3xl text-white">{order.order_number}</h1>
              ...
              <p className="text-fg-muted text-sm italic">{t('placed_on')} {new Date(order.created_at).toLocaleDateString(locale)}</p>
            </div>
          ...
          {returnRequest && (
            <div className={`glass-effect p-6 rounded-3xl border border-[#f4c025]/20 bg-[#f4c025]/5 mb-10 flex items-center justify-between ${rtl ? 'flex-row-reverse' : ''}`}>
              <div className={`flex items-center gap-4 ${rtl ? 'flex-row-reverse' : ''}`}>
                ...
                <div>
                  <h2 className="text-white font-sans text-lg">{t('return_request')}</h2>
                  <p className={`text-fg-muted text-xs uppercase tracking-widest font-bold ${rtl ? 'text-right' : 'text-left'}`}>
                    {t('status')}: <span className="text-[#f4c025]">{t(`return_statuses.${returnRequest.status}`)}</span>
                  </p>
                </div>
              </div>
              <div className={rtl ? 'text-left' : 'text-right'}>
                <p className="text-fg-faint text-[10px] uppercase tracking-widest">{t('requested_on')}</p>
                <p className="text-fg-muted text-xs font-medium">{new Date(returnRequest.created_at).toLocaleDateString(locale)}</p>
              </div>
            </div>
          )}
          ...
          <div className="glass-effect p-8 rounded-3xl border border-white/5 mb-8">
            <h2 className={`text-lg font-sans text-white mb-6 ${rtl ? 'text-right' : 'text-left'}`}>{t('order_status')}</h2>
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              <div className="glass-effect p-8 rounded-3xl border border-white/5">
                <div className={`flex items-center gap-3 mb-6 ${rtl ? 'flex-row-reverse' : ''}`}>
                  ...
                  <h2 className="text-lg font-sans text-white">{t('order_items')}</h2>
                </div>
                  ...
                  {order.items?.map((item) => (
                    <div key={item.id} className={`py-4 flex justify-between items-center gap-4 ${rtl ? 'flex-row-reverse' : ''}`}>
                      <div>
                        ...
                        <p className={`text-fg-muted text-xs ${rtl ? 'text-right' : 'text-left'}`}>{item.selected_size} × {item.quantity}</p>
                      </div>
              ...
              {order.is_gift && order.gift_message && (
                <div className="glass-effect p-8 rounded-3xl border border-[#f4c025]/20 bg-[#f4c025]/5">
                  ...
                  <p className={`text-fg italic font-serif text-lg leading-relaxed ${rtl ? 'text-right' : 'text-left'}`}>{order.gift_message}</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="glass-effect p-6 rounded-3xl border border-white/5">
                <div className={`flex items-center gap-3 mb-4 ${rtl ? 'flex-row-reverse' : ''}`}>
                  <MapPin size={18} className="text-fg-muted" />
                  <h2 className="text-sm font-bold text-white uppercase tracking-widest">{t('shipping')}</h2>
                </div>
                {order.address ? (
                  <div className={`text-sm text-fg-muted space-y-1 ${rtl ? 'text-right' : 'text-left'}`}>
                  ...
                  <p className={`text-fg-muted text-xs ${rtl ? 'text-right' : 'text-left'}`}>{t('address_not_available')}</p>
                )}
              </div>
              ...
              <div className="glass-effect p-6 rounded-3xl border border-white/5">
                <div className={`flex items-center gap-3 mb-4 ${rtl ? 'flex-row-reverse' : ''}`}>
                  <CreditCard size={18} className="text-fg-muted" />
                  <h2 className="text-sm font-bold text-white uppercase tracking-widest">{t('payment')}</h2>
                </div>
                <p className={`text-fg-muted text-sm ${rtl ? 'text-right' : 'text-left'}`}>
                ...
                <p className={`text-fg-muted text-[10px] mt-2 uppercase tracking-widest ${rtl ? 'text-right' : 'text-left'}`}>{t('card_ending')}</p>
              </div>
            </div>
          </div>
        </div>
